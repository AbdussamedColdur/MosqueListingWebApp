                            ...
                            @foreach ($camiler as $index => $cami)
                                <div class="carousel-item {{ $index === 0 ? 'active' : '' }}">
                                    ...
                                    @if (!empty($cami['Photos'][0]['Base64']))
                                        <img class="w-100" src="data:image/jpg;base64,{{ $cami['Photos'][0]['Base64'] }}"
                                            alt="Decoded Image">
                                    @endif
                        ...
                        <div class="store-item position-relative text-center">
                            ...
                            @if (!empty($cami['Photos'][0]['Base64']))
                                <img style="width: 350px; height: 400px;"
                                    src="data:image/jpg;base64,{{ $cami['Photos'][0]['Base64'] }}" alt="Decoded Image">
                            @endif
                            ...
                            <div class="store-overlay">
                                <a href="/ltf/{{ $cami['Id'] }}" class="btn btn-primary rounded-pill py-2 px-4 m-2">Daha
                                    fazla Detay <i class="fa fa-arrow-right ms-2"></i></a>
                            </div>
                        </div>
                    </div>
                @endforeach
            ...
            <script>
                ...
                const ilceOptions = {
                    ...
                    foreach ($ilArrayIsim as $index => $ilName) {
                        $ilce = DB::table('İlceler')
                            ->where('cityid', $ilArrayId[$index])
                            ->select('name')
                            ->get();
                    ...
                    adInput.addEventListener('input', function() {
                        ...
                        if (!regex.test(inputValue)) {
                            ...
                            errorMessage.textContent =
                                'Yalnızca harf ve boşluk karakterleri kullanabilirsiniz.'; // Set the error message
                        } else {
